...
func (h *userHandler) SignUp(c *gin.Context) {
	...
	u, err := h.userInteractor.CreateUser(newUser)
	if err != nil {
	...
	helper.GenerateResponseAPI(http.StatusOK, "success", u, c, false)
}
...
func (h *userHandler) GetUserWithPosts(c *gin.Context) {
	...
	if err != nil {
		if errors.Is(err, user.ErrorUserNotFound) {
			log.Printf("failed to get user by id, id: %d, err: %s", userId, err)
			helper.GenerateResponseAPI(http.StatusNotFound, "error", "User not found", c, false)
			return
		}
		helper.GenerateResponseAPI(http.StatusInternalServerError, "error", "Unknown error occurred", c, false)
